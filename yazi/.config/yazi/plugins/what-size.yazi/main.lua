...
--- @param use_hovered boolean Use hovered item instead of CWD when nothing selected
--- @return paths table Table of selected urls
local function get_paths(selected, use_hovered)
    -- If no files are selected, get hovered item or current directory
    if #selected == 0 then
        local paths = {}
        if use_hovered then
...
return {
    entry = function(self, job)
        ...
        local use_hovered = job.args.hovered or job.args[1] == '-h' or job.args[2] == '-h'
        ...
        local items = get_paths(selected, use_hovered)
